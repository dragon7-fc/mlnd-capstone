...
class SSDViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    @IBAction func zoomSliderValueChanged(_ sender: UISlider) {
        zoomLabel.text = String(Int(sender.value)) + "x"
        
        cameraManager.zoom(CGFloat(sender.value))
    }
    ...
    let ssdPostProcessor = SSDPostProcessor(numAnchors: 1917, numClasses: 1)  //fc-test*
...
//    }()
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
//        self.cameraView?.layer.addSublayer(self.cameraLayer)  //fc-test-
        //fc-test+[
        cameraManager.addPreviewLayerToView(self.cameraView)
        
        cameraManager.resumeCaptureSession()
        self.captureSession = cameraManager.captureSession
        
        self.cameraView?.addSubview(self.frameLabel)
        self.frameLabel.layer.zPosition = 1
        
        self.cameraView?.addSubview(self.zoomLabel)
        self.zoomLabel.layer.zPosition = 1
        
        self.cameraView?.addSubview(self.zoomSlider)
        self.zoomSlider.layer.zPosition = 1
        //fc-test+]
//        self.cameraView?.bringSubview(toFront: self.frameLabel)  //fc-test-
        self.frameLabel.textAlignment = .left
        let videoOutput = AVCaptureVideoDataOutput()
        videoOutput.setSampleBufferDelegate(self, queue: DispatchQueue(label: "MyQueue"))
        self.captureSession?.addOutput(videoOutput)
//        self.captureSession.startRunning()  //fc-test-
        setupVision()
...
//    }
    ...
    func setupVision() {
        guard let visionModel = try? VNCoreMLModel(for: ssd_mobilenet_v1_egohands_feature_extractor().model)
    ...
    func processClassifications(for request: VNRequest, error: Error?) -> [Prediction]? {
        ...
        guard let boxPredictions = results[0].featureValue.multiArrayValue,  //fc-test
            let classPredictions = results[1].featureValue.multiArrayValue else {  //fc-test
            return nil
        }
    ...
    func drawBoxes(predictions: [Prediction]) {
        ...
        for (index, prediction) in predictions.enumerated() {
            if let classNames = self.ssdPostProcessor.classNames {
                ...
                let className = classNames[prediction.detectedClass]  //fc-test+
                let textLabel = String(format: "%@ - %.2f", className, self.sigmoid(prediction.score))  //fc-test*
                
                //fc-test+[
                let utterance = AVSpeechUtterance(string: classNames[prediction.detectedClass])
